...
def fuse_labels(seg_dir, merge_CWM=False):
    # Merge white matters
...
def get_islands(label_map_dir, save_map, threshold=None):
    ...
    for label_map_path in seg_paths:
        ...
        # mark all the islands for every label except the background: Done
            ...
            for j in range(y):
                for k in range(z):
                    if islands[i, j, k] != 0:
                        ...
                        islands[i, j, k] = 0
        save_name = subject_paths[subject].split('/')[-1].split('_')[0] + "_" + subject_paths[subject].split('/')[-1].split('_')[1] + "_islands_removed.nii.gz"
        print("save to: ")
        print(save_name)
...
# fuse labels
# fuse_labels(seg_dir=r"/home/ziyaos/testEP_masked/BEE_plus/merged_seg", # The path to the segmentation maps, the fused maps will be saved in place
#             merge_CWM=False) # Whether or not to merge the Cerebellum white matter and grey matter. [7]->[8] and [46]->[47]

# get and save the islands
get_islands(label_map_dir=r"/home/ziyaos/testEP_masked/primate/mixed/predict/T1seg", # the path to the segmentation maps
            save_map=r"/home/ziyaos/testEP_masked/primate/mixed/predict/islands", # where to save the map containing the islands
            threshold=15) # components larger than the threshold will not be modified

# clear islands given island maps
clear_islands(label_dir="/home/ziyaos/testEP_masked/primate/mixed/predict/T1seg", # the path to the segmentation labels
              island_dir="/home/ziyaos/testEP_masked/primate/mixed/predict/islands", # the path to the island maps
              posterior_dir="/home/ziyaos/testEP_masked/primate/mixed/predict/T1post", # the path to the merged posteriors
              save_dir="/home/ziyaos/testEP_masked/primate/mixed/predict/island_removed", # the directory to the save the final label maps
              labels=[0, 14, 15, 16, 24, 85, 86, 2, 3, 4, 5, 7, 8, 10, 11, 12, 13,
                    17, 18, 26, 28, 30, 31, 41, 42, 43, 44, 46, 47, 49, 50, 51, 52, 53, 54, 58, 60, 62, 63]) # the list of labels the corresponds to the posteriors (the segmention labels when predicting)
